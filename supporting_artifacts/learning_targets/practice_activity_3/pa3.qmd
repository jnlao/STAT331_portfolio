...
6.  Less than 1,000 people apply to this college every year. (Assume the size of the first year class is 1/4 of the undergraduate population.)
    ...
    1.  mutate()
        ...
        2.  application_num = freshman/ADM_RATE (use for filter step)
...
mystery_college <- colleges_clean |> 
  filter(REGION == 7) |>
  mutate(reg_med_rate = median(ADM_RATE)) |> 
  filter(TUITION_DIFF != 0, 
         SAT_AVG %% 2 == 1, 
         STABBR != 'ID') |> 
  mutate(fresh_amt = as.numeric(UGDS)/4,
         app_num = fresh_amt/ADM_RATE) |> 
  filter(app_num < 1000,
         STABBR != 'MT')
  # cheapest is Fort Lewis College
